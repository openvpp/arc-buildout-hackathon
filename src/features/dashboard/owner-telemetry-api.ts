...
export type OwnerTelemetryResponse = z.infer<typeof responseSchema>;
export type OwnerVerifyResponse = z.infer<typeof verifyResultSchema>;

/** @deprecated Use OwnerTelemetryResponse */
export type DemoTelemetryResponse = OwnerTelemetryResponse;
/** @deprecated Use OwnerVerifyResponse */
export type DemoVerifyResponse = OwnerVerifyResponse;

/**
 * Owner dashboard purchase + verify client. BFF settles with server Circle buyer
 * (or mock). Verify runs independent Arc + content-hash evidence.
 * Requires Web3Auth id token for device-owner auth.
 *
 * Routes remain under `/api/v1/demo/telemetry/*` for compatibility.
 */
export function createOwnerTelemetryApi(client: ApiClient = new ApiClient()) {
  return {
    async quote(input: {
      idToken: string;
...
/** @deprecated Use createOwnerTelemetryApi */
export const createDemoTelemetryApi = createOwnerTelemetryApi;
...
): Promise<OwnerTelemetryResponse> {
